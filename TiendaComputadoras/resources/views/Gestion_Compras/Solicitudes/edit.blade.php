...
@section('content')
    <div class="row">
        <section class="col-md-12 col-xl-12">
            <div class="card mb-12">
                <div class="card-body ">
                    <form class="row" action="{{ route('solicitud.update',$solicitudes->id) }}" id="formulario" method="POST"
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" id="fecha" name="fecha"
                                    ...
                                    value="{{ old('fecha', $solicitudes->fecha_entrega_esperada) }}" required>
                                @error('fecha')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea id="descripcion" name="descripcion" rows="3"
                                    class="form-control @error('descripcion') is-invalid @enderror">{{ old('descripcion', $solicitudes->notas) }}</textarea>
                                @error('descripcion')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-12">
                            <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-2">
                                ...
                                <button id="registrar" type="submit" class="btn btn-primary mb-2" disabled>Registrar
                                    Solicitud</button>
                                <button type="button" id="agregar" class="btn btn-secondary mb-2">Agregar a la
                                    Solicitud</button>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            cancelarBtn.addEventListener('click', limpiarLocalStorage);
        });
        ...
        function agregarDetallesAlLocalStorage() {
            ...
            for (let i = 0; i < productoSelect.options.length; i++) {
                ...
                // Obtener atributos data-* de cada opción
                const modelo = option.getAttribute('data-modelo');
                const codigo = option.getAttribute('data-codigo');
                const nombre = option.getAttribute('data-nombre');
                const color = option.getAttribute('data-color');
                const corte = option.getAttribute('data-corte');
                const talla = option.getAttribute('data-talla');
                const marca = option.getAttribute('data-marca');

                // Buscar detalles que coincidan con productosdetalles_id igual a id
                const detalleEncontrado = detalles.find(detalle => detalle.productosdetalles_id == id);

                if (detalleEncontrado) {
                    // Crear objeto de producto a guardar en localStorage
                    const producto = {
                        id: id,
                        nombre: nombre,
                        marca: marca,
                        ...
                        corte: corte,
                        Talla: talla,
                        cantidad: detalleEncontrado.cantidad
                    };

                    // Verificar si el producto ya está en localStorage
                    const productoExistenteIndex = productosEnLocalStorage.findIndex(p => p.id == producto.id);

                    if (productoExistenteIndex !== -1) {
                        // Si el producto existe, actualizar la cantidad
                        productosEnLocalStorage[productoExistenteIndex].cantidad += detalleEncontrado.cantidad;
                    } else {
