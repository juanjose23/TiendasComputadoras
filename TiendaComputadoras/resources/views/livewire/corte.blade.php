            ...
            <tbody>
                ...
                @foreach ($cortes as $corte)
                    <tr>
                        ...
                        <td><span class="badge rounded-pill {{ $corte->estado == 1 ? 'bg-success' : 'bg-danger' }}">
                        ...
                        <td>
                            ...
                            <form id="deleteForm{{ $corte->id }}"
                                action="{{ route('cortes.destroy', ['cortes' => $corte->id]) }}" method="POST">
                                @csrf
                                @method('DELETE')
            ...
            <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                <div class="btn-group me-2" role="group" aria-label="First group">
                    ...
                    <button type="button" class="btn btn-primary" wire:click="previousPage"
                        {{ $cortes->onFirstPage() ? 'disabled' : '' }}>
                        &laquo; Previo
                    </button>
                </div>

                <!-- Botones para cada página -->
                @foreach ($cortes->links() as $page => $url)
                    @if (
                        $loop->first ||
                            $loop->last ||
                            ($loop->index >= $cortes->currentPage() - 2 && $loop->index <= $cortes->currentPage() + 2))
                    ...
                    @elseif ($loop->index == $cortes->currentPage() - 3 || $loop->index == $cortes->currentPage() + 3)
...
<script>
    function confirmAction(CorteId) {
        Swal.fire({
            title: '¿Estás seguro?',
            text: '¿Quieres cambiar el estado de este corte?',
            icon: 'warning',
