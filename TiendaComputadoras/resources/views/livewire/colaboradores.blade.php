            ...
            <table class="table mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $colaborador->persona_natural->apellido}}</td>
                        <td>{{ $colaborador->persona_natural->tipo_identificacion}}</td>
                        <td>{{ $colaborador->persona_natural->identificacion}}</td>
                        <td>{{ $colaborador->telefono}}</td>
                        <td>{{ $colaborador->correo}}</td>
                        <td><span class="badge rounded-pill {{ $colaborador->empleados->estado == 1 ? 'bg-success' : 'bg-danger' }}">
                        ...
                        <td>
                            ...
                            <button type="button"
                                class="btn btn-{{ $colaborador->estado == 0 ? 'danger' : 'success' }} d-block" role="button"
                                onclick="confirmAction({{ $colaborador->id }})">
                                <i class="bi bi-{{ $colaborador->estado == 1 ? 'trash' : 'power' }}"></i>
                                {{ $colaborador->estado == 0 ? 'Desactivar' : 'Activar' }}
                            </button>


                            <form id="deleteForm{{ $colaborador->id }}"
                                action="{{ route('colaboradores.destroy', ['colaboradores' => $colaborador->empleados->id]) }}" method="POST">
                                @csrf
                                @method('DELETE')
